...
def recuperarArchivo(img1, img2):
	if os.path.isfile(img1) and os.path.isfile(img2):
		...
		for y in range(0, height): # Por cada fila
		    for x in range(0, width): # Por cada columna
		        if pix_original[x,y][0] != pix_intercepted[x,y][0]: # Si el pixel es distinto
		        	ba.append(True) # Es un 1 en el array
		        else:
		        	ba.append(False) # 0 en caso contrario
		        aux = aux + 1
		with open("fichero1", 'wb') as fh:
		    ba.tofile(fh) # Guardamos los bits obtenidos en un archivo
		ba = bitarray.bitarray() # Creamos un array de bits
		aux = 0;
		for y in range(0, height): # Por cada fila
		    for x in range(0, width): # Por cada columna
		        if pix_original[x,y][1] != pix_intercepted[x,y][1]: # Si el pixel es distinto
		        	ba.append(True) # Es un 1 en el array
		        else:
		        	ba.append(False) # 0 en caso contrario
		        aux = aux + 1
		with open("fichero2", 'wb') as fh:
		    ba.tofile(fh) # Guardamos los bits obtenidos en un archivo
		ba = bitarray.bitarray() # Creamos un array de bits
		aux = 0;
		for y in range(0, height): # Por cada fila
		    for x in range(0, width): # Por cada columna
		        if pix_original[x,y][2] != pix_intercepted[x,y][2]: # Si el pixel es distinto
		        	ba.append(True) # Es un 1 en el array
		        else:
		        	ba.append(False) # 0 en caso contrario
		        aux = aux + 1
		with open("fichero3", 'wb') as fh:
		    ba.tofile(fh) # Guardamos los bits obtenidos en un archivo
		exit(0)
	else:
...
if __name__ == "__main__":
	if len(sys.argv) == 3:
		recuperarArchivo(sys.argv[1], sys.argv[2])
	else:
